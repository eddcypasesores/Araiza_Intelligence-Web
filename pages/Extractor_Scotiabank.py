...
LOGO_RIGHT = ASSETS_DIR / "banks/scotiabank.jpeg"
...
st.set_page_config(page_title="Extractor Scotiabank", layout="centered")
...
with col2:
    st.markdown(
        "<h2 style='text-align:center;margin:0;'>Extractor de Estado de Cuenta Scotiabank</h2>",
        unsafe_allow_html=True,
    )
with col3:
...
uploaded = st.file_uploader("PDF de Scotiabank", type=["pdf"])
...
with st.spinner("Extrayendo movimientos..."):
    df = extraer_scotiabank(uploaded)
...
with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
    df.to_excel(writer, sheet_name="Scotiabank", index=False)
buffer.seek(0)

filename = f"scotiabank_{datetime.now():%Y%m%d_%H%M%S}.xlsx"
st.download_button(
    "Descargar Excel",
    data=buffer,
    file_name=filename,
