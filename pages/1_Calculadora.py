...
def _swap_point_data(role_a: str, role_b: str) -> None:
    ...
    snapshot: dict[str, dict[str, Any]] = {
        role: _capture_point_state(prefix) for role, prefix in role_prefix.items()
    }
...
with st.container():
    ...
    with col_addresses:
        ...
        stored_roles = st.session_state.get("route_roles", ["origin", "destination"])
        allowed_roles = ("origin", "stop", "destination")
        route_roles = [r for r in stored_roles if r in allowed_roles]

        def _ensure_role(order: list[str], role: str, *, position: int | None = None) -> None:
            if role in order:
                return
            if position is None or position >= len(order):
                order.append(role)
                return
            order.insert(position, role)

        if show_stop:
            if not route_roles:
                route_roles = ["origin", "stop", "destination"]
            else:
                _ensure_role(route_roles, "origin", position=0)
                stop_position = 1 if "origin" in route_roles else 0
                _ensure_role(route_roles, "stop", position=stop_position)
                _ensure_role(route_roles, "destination")
        ...
        st.session_state["route_roles"] = list(route_roles)
