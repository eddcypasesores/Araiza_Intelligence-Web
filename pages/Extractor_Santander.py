...
from core.extractor_santander import extraer_santander

ASSETS_DIR = next((p for p in (Path("Assets"), Path("assets")) if p.exists()), Path("."))
LEFT_LOGO = ASSETS_DIR / "logo.jpg"
RIGHT_LOGO = ASSETS_DIR / "banks/santander.jpeg"
...
def _df_to_excel_bytes(df: pd.DataFrame) -> bytes:
    ...
    with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
        df.to_excel(writer, index=False, sheet_name="Santander")
    buffer.seek(0)
...
def _render_header() -> None:
    ...
    with c2:
        st.markdown("<h2 class='brand-title'>EXTRACTOR SANTANDER</h2>", unsafe_allow_html=True)
    with c3:
        if right_logo:
            st.markdown(f"<img src='{right_logo}' class='logo-img' alt='Santander logo'>", unsafe_allow_html=True)


st.set_page_config(page_title="Extractor Santander", layout="wide")
apply_theme()
...
st.caption("Sube tu PDF de Santander y generaremos un Excel con los movimientos detectados.")
...
if uploaded is None:
    ...
    try:
        ...
        if df.empty:
            ...
            filename = f"movimientos_santander_{timestamp}.xlsx"
