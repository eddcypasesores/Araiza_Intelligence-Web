...
LOGO_RIGHT = ASSETS_DIR / "banks/santander.jpeg"
...
st.set_page_config(page_title="Extractor Santander", layout="centered")
...
with col2:
    st.markdown(
        "<h2 style='text-align:center;margin:0;'>Extractor de Estado de Cuenta Santander</h2>",
        unsafe_allow_html=True,
    )
with col3:
...
uploaded = st.file_uploader("PDF de Santander", type=["pdf"])
...
with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
    df.to_excel(writer, sheet_name="Santander", index=False)
buffer.seek(0)

filename = f"santander_{datetime.now():%Y%m%d_%H%M%S}.xlsx"
st.download_button(
    "Descargar Excel",
    data=buffer,
    file_name=filename,
