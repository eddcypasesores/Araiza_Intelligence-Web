...
# Parámetros
MAX_MB = 600           # ligado a --server.maxUploadSize en deploy
DEFAULT_BATCH = 150    # lotes pequeños = menos picos RAM/CPU
POLL_INTERVAL = 0.7    # s
...
def parse_emisor_from_xml_stream(fp) -> tuple[str | None, str | None, str | None, float | None, str]:
    """Extrae Emisor RFC/Nombre, Fecha, Total y Estatus sin cargar todo el XML."""
    try:
        fecha = None
        total = None
...
# =============================================================================
# Excel builder (Coincidencias / Desglose)
# =============================================================================
...
def build_excel_bytes(xml_source: pd.DataFrame | Path | str, df_black: pd.DataFrame) -> bytes:
    ...
    if not coincidencias.empty:
        ...
        if "FECHA DE PRIMERA PUBLICACION" in coincidencias_ordenadas.columns:
            coincidencias_ordenadas = coincidencias_ordenadas.sort_values("FECHA DE PRIMERA PUBLICACION", ascending=False)
...
# =============================================================================
# Firmes/Exigibles — autodetección + manifest y subida directa en esta página
# =============================================================================
...
# =============================================================================
# SQLite / Indexación de XML (streaming)
# =============================================================================
...
# =============================================================================
# UI — (2) Cargar Firmes/Exigibles DESDE AQUÍ (misma página)
# =============================================================================
st.markdown('<div class="card">', unsafe_allow_html=True)
st.markdown('<div class="hdr">2) Cargar archivos SAT (Firmes / Exigibles)</div>', unsafe_allow_html=True)
...
with colF:
    st.caption("Sube un archivo de **Firmes** (.csv/.xlsx/.xls). Debe incluir columna `RFC`.")
    ...
    if up_firmes is not None:
        if not _has_rfc_column_in_upload(up_firmes):
            st.error("El archivo de Firmes no parece tener columna `RFC`.")
        else:
...
with colE:
    st.caption("Sube un archivo de **Exigibles** (.csv/.xlsx/.xls). Debe incluir columna `RFC`.")
    ...
    if up_exig is not None:
        if not _has_rfc_column_in_upload(up_exig):
            st.error("El archivo de Exigibles no parece tener columna `RFC`.")
        else:
...
tengo_blacklist = _has_rfc_column(ss.get("blacklist_df"))
if not (tengo_firmes and tengo_exigibles):
    st.warning("Para habilitar el cruce, sube **Firmes** y **Exigibles** (ambos deben tener columna `RFC`).")

# =============================================================================
# UI — (1) Carga masiva (ZIP) e indexación
# =============================================================================
st.markdown('<div class="card">', unsafe_allow_html=True)
...
# =============================================================================
# Cruce y descarga de Excel (habilitado cuando hay index y listas)
# =============================================================================
tengo_blacklist = _has_rfc_column(ss.get("blacklist_df"))
...
if tengo_xml and tengo_blacklist:
    ...
    if tengo_xml and (not tengo_blacklist):
        st.info("Sube **Firmes** y **Exigibles** en la sección 2 para habilitar la descarga de Excel.")
