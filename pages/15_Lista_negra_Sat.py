...
# --- INICIO: CODIGO CORREGIDO (AMBAS CORRECCIONES) ---
...
def _zip_job_worker(job_id: str, zip_data: bytes, zip_path: Path, batch: int) -> None:
    ...
    try:
        ...
        # --- MODIFICADO para capturar 'skipped' ---
        total, inserted, skipped, secs = bulk_index_zip(zip_path, XML_DB_PATH, progress_cb=_progress, batch=batch)
        ...
        _zip_job_update(
            ...
            total=total,
            inserted=inserted,
            skipped=skipped, # <-- Añadido al reporte del job
            progress=1.0,
            seconds=secs,
            finished=time.time(),
        )
...
# --- INICIO: CODIGO CORREGIDO (bulk_index_zip) ---
...
def bulk_index_zip(
    ...
    *,
    progress_cb=None,
    batch: int = 200,
) -> tuple[int, int, int, float]: # <-- Modificada la firma de retorno
    _db_init(db_path)
    t0 = time.time()

    # --- LÍMITE DE SEGURIDAD ---
    # Define un tamaño máximo por XML (ej. 50MB) para evitar crashes por OOM
    # Ajusta este valor según la RAM de tu servidor
    MAX_XML_SIZE_MB = 50
    MAX_XML_SIZE_BYTES = MAX_XML_SIZE_MB * 1024 * 1024
    # --- FIN LÍMITE ---
    ...
    processed = 0
    inserted = 0
    skipped_large = 0 # <-- Nuevo contador
    total_files = 0
    ...
    try:
        ...
        with zipfile.ZipFile(stream) as zf, closing(sqlite3.connect(str(db_path))) as con:
            ...
            for info in files:
                ...
                # --- NUEVO CHEQUEO DE TAMAÑO ---
                if info.file_size > MAX_XML_SIZE_BYTES:
                    skipped_large += 1
                    processed += 1 # Contar como "procesado" para la barra de progreso
                    if progress_cb:
    ...
    return processed, inserted, skipped_large, duration

# --- FIN: CODIGO CORREGIDO (bulk_index_zip) ---
...
# --- INICIO: CODIGO CORREGIDO (_on_zip_selected) ---
...
# --- FIN: CODIGO CORREGIDO (_on_zip_selected) ---
...
if job_info:
    ...
    skipped = int(job_info.get("skipped") or 0) # <-- OBTENER OMITIDOS

    # --- MODIFICADO para mostrar omitidos ---
    caption_msg = f"Procesando XML {processed:,} de {total:,} (insertados {inserted:,}"
    if skipped > 0:
        caption_msg += f", omitidos {skipped:,}"
    caption_msg += ")"
    ...
    if status in {"queued", "running"}:
        ...
        ss["zip_last_summary"] = {
            ...
            "insertados": inserted,
            "omitidos": skipped, # <-- GUARDAR OMITIDOS
...
if show_metrics:
    ...
    if summary:
        # --- MODIFICADO para mostrar omitidos ---
        caption_msg = f"Ultima carga: {summary['procesados']:,} XML procesados, {summary['insertados']:,} nuevos"
        skipped_sum = summary.get("omitidos", 0)
        if skipped_sum > 0:
            caption_msg += f", {skipped_sum:,} omitidos (muy grandes)"
        caption_msg += f", {summary['tiempo']:.1f}s."
        st.caption(caption_msg)
        # --- FIN MODIFICACION ---
