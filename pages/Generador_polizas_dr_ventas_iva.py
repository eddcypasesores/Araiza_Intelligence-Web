"""Generador para Pólizas Dr Ventas con IVA (sin fila 7)."""
...
st.set_page_config(
    page_title="Pólizas Dr Ventas con IVA",
    page_icon="📄",
    layout="wide",
...
def write_block_sin7(
    ...
    ws_out.cell(row=r + 2, column=7, value=f"=F{r+1}/1.16")
...
def render_tab(tab_label, key_prefix, cod_ingresos_override=None, cod_iva_tras_override=None):
    st.subheader(f"Proceso {tab_label} (sin fila 7)")
    ...
    if uploaded:
        if st.button("Generar archivo apilado", key=f"{key_prefix}_btn"):
            try:
                data = build_stacked_output_sin7(
                    uploaded.getvalue(),
