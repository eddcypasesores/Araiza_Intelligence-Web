...
def _render_nav() -> None:
    render_brand_logout_nav(
        ...
        brand="Extractor Inbursa",
        action_label="Atras",
        action_href=_back_href(),
    )


st.set_page_config(page_title="Extractor Inbursa", layout="centered")
apply_theme()
...
with col2:
    st.markdown("<h2 style='text-align:center;margin:0;'>Extractor de Estado de Cuenta Inbursa</h2>", unsafe_allow_html=True)
with col3:
...
uploaded = st.file_uploader("PDF de Inbursa", type=["pdf"])

if uploaded:
    with st.spinner("Extrayendo movimientos..."):
        df = extraer_inbursa(uploaded)
    ...
    with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
        df.to_excel(writer, sheet_name="Inbursa", index=False)
    buffer.seek(0)

    fname = f"inbursa_{datetime.now():%Y%m%d_%H%M%S}.xlsx"
    st.download_button(
        "Descargar Excel",
        data=buffer,
        file_name=fname,
